...
AQUA_NAMESPACE::ShadowPipeline::ShadowPipeline(vkLib::PShader shader, vkLib::Framebuffer depthBuffer, const VertexBindingMap& vertexBindings)
{
	...
	SetupConfig(config, depthBuffer.GetResolution(), vertexBindings);
...
void AQUA_NAMESPACE::ShadowPipeline::UpdateCamera(CameraBuf camera)
{
	vkLib::StorageBufferWriteInfo storageInfo{};
	storageInfo.Buffer = camera.GetNativeHandles().Handle;

	this->UpdateDescriptor({ 0, 0, 0 }, storageInfo);
}

void AQUA_NAMESPACE::ShadowPipeline::UpdateModels(Mat4Buf models)
{
	vkLib::StorageBufferWriteInfo storageInfo{ models.GetNativeHandles().Handle };
	this->UpdateDescriptor({ 0, 1, 0 }, storageInfo);
}

void AQUA_NAMESPACE::ShadowPipeline::SetupConfig(vkLib::GraphicsPipelineConfig& config, const glm::uvec2& scrSize, const VertexBindingMap& bindings)
{
