...
public: // should be public
	AQUA_API ShadingPipeline(const vkLib::PShader& shader, vkLib::Framebuffer shadingBuffer);
	...
	AQUA_API void SetGeometry(const FragmentResourceMap& geometry);
	AQUA_API void SetGeometryDescLocation(const std::string& tag, const glm::uvec3& location);
...
inline ShadingPipeline Clone(vkLib::Context ctx, const ShadingPipeline& pipeline)
{
	return ctx.MakePipelineBuilder().BuildGraphicsPipeline<ShadingPipeline>(pipeline);
}
